...
fileURIPromise.then(function(file){var data={companyId:file.id,riskFactor:textFields.riskFactor.val(),managementDisc:textFields.managementDis.val(),finStateSuppData:textFields.finanState.val()};var result=sendOutTextArea(data);if($.cookie("countTask")>=9)result.then(function(data){if(data.countTask==10)allComplete("You have completed ten tasks.",data.commToken)})})}else alertify.alert("One or multiple textareas aren't filled."+" If the document is corrupted or doesn't contain all three fields, "+
"please click the red button <span class='red'>Unable to Complete</span> "+" at the bottom of the page. ")})})();(function autoFillmTurkId(){turkerIdentity.then(function(data){if(data.mturkId){$("#MturkId").val(data.mturkId);$("input").prop("disabled",true)}if(data.countTask)animate.animateProgressBar(data.countTask)})})();function updateDocLinks(promise){if(promise)promise.then(function(data){$("#readInBrowser").attr("href",data.htmlURL);$("#downloadFile").attr("href",data.txtURL)})}function saveMTurkIdEvent(){if(!$("#MturkId").val())alertify.alert("<span class='red'>Warning: </span> You must type in your MTurk Id before saving it!");
else if($("input").prop("disabled"))alertify.alert("<span class='red'>Warning: </span> Your mTurk ID already exists. If there is any error, please email anie@emory.edu");else{glo.savemTurkID($("#MturkId").val());$("input").prop("disabled",true);$.cookie("mturkId",$("#MturkId").val())}}function allComplete(customMessage,commToken){alertify.alert("<div class='row'><div class='large-12 columns'><h4>Awesome!</h4></div></div>"+"<div class='row'><div class='large-12 columns'><p>"+customMessage+" Close this page and go back to"+
" mTurk now! You have one minute to copy and paste following string to your Amazon mTurk window:</p></div></div>"+"<div class='row text-center'><div class='large-12 columns'><kbd>"+commToken+"</kbd></div></div><br><br>");glo.restart();$("#alertify-ok").click(function(event){window.location=baseHostName});setTimeout(function(){window.location=baseHostName},6E4)}function rejectMessage(){alertify.alert("<div class='row'><div class='large-12 columns'><h4>Sorry!</h4></div></div>"+"<div class='row'><div class='large-12 columns'><p>We don't have more tasks for you right now! Close this page and go back to"+
" mTurk now! We apologize for this inconvenience! </p></div></div><br>");$("#alertify-ok").click(function(event){window.location=baseHostName});setTimeout(function(){window.location=baseHostName},6E4)}function loadNewDocURL(){if(document.URL.indexOf("turk")>=0)return companyURIPromise.then(function(uris){return Q($.ajax({url:uris.webOneCompany._2,type:uris.webOneCompany._1,datatype:"json",contentType:"application/json; charset=UTF-8"})).then(function(data){return data}).fail(function(jqXHR){if(jqXHR.responseText==
JSON.stringify("There is no document fitting requirements left")){var data=glo.refresh();data.then(function(mturker){if(mturker.countTask>=1){var message="You are the lucky one! It turns out we don't have more documents for you to complete! Congrats!";allComplete(message,$.cookie("commToken"))}else rejectMessage()})}else glo.ajaxFailureHandle(jqXHR,uris.webOneCompany._2)})})}function unableToComplete(data){return companyURIPromise.then(function(uris){return Q($.ajax({url:uris.webUnableToComp._2,type:uris.webUnableToComp._1,
datatype:"json",data:JSON.stringify(data),contentType:"application/json; charset=UTF-8"})).then(function(data){return data}).fail(function(jqXHR){glo.ajaxFailureHandle(jqXHR,uris.webUnableToComp._2)})})}function sendOutTextArea(data){return companyURIPromise.then(function(uris){return Q($.ajax({url:uris.webUpload._2,type:uris.webUpload._1,datatype:"json",contentType:"application/json; charset=UTF-8",data:JSON.stringify(data)})).then(function(data){return oneTaskCompletMTurk()}).fail(function(jqXHR){glo.ajaxFailureHandle(jqXHR,
uris.webUpload._2)})})}function oneTaskCompletMTurk(){return mTurkURIPromise.then(function(uris){return Q($.ajax({url:uris.taskComplete._2,type:uris.taskComplete._1,datatype:"json",contentType:"application/json; charset=UTF-8"})).then(function(data){$.cookie("countTask",data.countTask);animate.taskComplete(data.countTask);textFields.riskFactor.val("");textFields.managementDis.val("");textFields.finanState.val("");return data}).fail(function(jqXHR){glo.ajaxFailureHandle(jqXHR,uris.taskComplete._2)})})}
return{fileURIPromise:fileURIPromise}}(jQuery,global_access,animate,alertify);