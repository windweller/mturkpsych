...
var global_access=function($,window,loc,alertify){if(!window.location.origin)window.location.origin=window.location.protocol+"//"+window.location.host;var baseHostName=window.location.origin;var mTurkURIPromise=mTurkURIretrieve();var companyURIPromise=companyURIretrieve();var mTurkIdentity=_establishIdentity();function refresh(){$.removeCookie("countTask");return _establishIdentity()}function restart(){$.removeCookie("commToken");$.removeCookie("countTask");return _establishIdentity()}function _establishIdentity(){return mTurkURIPromise.then(function(mTurkUris){return Q($.ajax({url:mTurkUris.mTurkerGet._2,
type:mTurkUris.mTurkerGet._1,dataType:"JSON"})).then(function(data){if(data.mturkId&&!$.cookie("mturkId")){$.cookie("mturkId",data.mturkId);$("input").prop("disabled",true);$("#MturkId").val($.cookie("mturkId"))}if(!data.mturkId&&$.cookie("mturkId")){var html="<div class='row'><div class='large-12 columns'>"+"<p>We detect that you have already typed in a mTurk ID: <kbd>"+$.cookie("mturkId")+"</kbd></p>"+"<p>If you are the owner of this mTurk ID, click <kbd>Cancel</kbd>. You won't have to type it in again."+
" However, if you are not, or if you are using a public computer, click <kbd>OK</kbd> so we can successfully proceed."+" Bare in mind if you click <kbd>OK</kbd>, your progress will be completely lost.</p>"+"<p> You might also be seeing this message if you are here for the second time. If so, you are welcome! Please click <kbd>Cancel</kbd>. </p>"+"</div></div><br>";alertify.confirm(html,function(res){if(res){$.removeCookie("mturkId");$.removeCookie("commToken");$.removeCookie("countTask");loc.reload()}else savemTurkID($.cookie("mturkId"))})}$.cookie("countTask",
data.countTask);return data}).fail(function(jqXHR){customAjaxFailureHandle(jqXHR,mTurkUris.mTurkerGet._2,function(html){var response=html+"<div class='row'><div class='large-12 columns'><div data-alert class='alert-box info radius'> Click 'OK' button, we can solve this problem "+"for you by resetting your COOKIE. However, doing so will eliminate your current progress. <a href='#'' class='close'>&times;</a></div></div></div>";alertify.confirm(response,function(res){if(res){$.removeCookie("commToken");
$.removeCookie("countTask");$.removeCookie("mturkId");loc.reload()}})})})})}function mTurkURIretrieve(){return Q($.ajax({url:baseHostName+"/mturker/help",type:"GET",dataType:"JSON"})).then(function(data){return data}).fail(function(jqXHR){ajaxFailureHandle(jqXHR,baseHostName+"/mturker/help")})}function companyURIretrieve(){return Q($.ajax({url:baseHostName+"/sec/help",type:"GET",dataType:"JSON"})).then(function(data){return data}).fail(function(jqXHR){ajaxFailureHandle(jqXHR,baseHostName+"/sec/help")})}
function savemTurkID(id){var data={"mturkid":id};mTurkURIPromise.then(function(uris){Q($.ajax({url:uris.updateMTurkId._2,type:uris.updateMTurkId._1,datatype:"json",contentType:"application/json; charset=UTF-8",data:JSON.stringify(data)})).fail(function(jqXHR){glo.ajaxFailureHandle(jqXHR,uris.updateMTurkId._2)})})}function _ajaxErrorMsgConstructor(jqXHR,url){var errorMessage="An error has occured from "+url+": "+jqXHR.status+" - "+jqXHR.statusText+". response text: "+jqXHR.responseText;return{txt:errorMessage,
html:"<div class='row'><div class='large-12 columns'><h4>An error has occured</h4></div></div>"+"<div class='row'><div class='large-12 columns'><p>Please copy and paste the following information to anie@emory.edu. Thank you!</p></div></div>"+"<div class='row'><div class='large-12 columns'><p>"+errorMessage+"</p></div></div>"+"<div class='row'><div class='large-12 columns'><p>CommToken: "+$.cookie("commToken")+" <br> CountTask: "+$.cookie("countTask")+"</p></div></div>"}}function ajaxFailureHandle(jqXHR,
url){var errorMsgs=_ajaxErrorMsgConstructor(jqXHR,url);console.error(errorMsgs.txt);alertify.alert(errorMsgs.html)}function customAjaxFailureHandle(jqXHR,url,callback){var errorMsgs=_ajaxErrorMsgConstructor(jqXHR,url);console.error(errorMsgs.txt);callback(errorMsgs.html)}return{mTurkIdentity:mTurkIdentity,mTurkURIPromise:mTurkURIPromise,companyURIPromise:companyURIPromise,savemTurkID:savemTurkID,ajaxFailureHandle:ajaxFailureHandle,customAjaxFailureHandle:customAjaxFailureHandle,refresh:refresh,restart:restart}}(jQuery,
window,location,alertify);
