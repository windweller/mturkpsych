...
class DelayedDiscountService(delayedDiscountActor: ActorRef)(implicit system: ActorSystem)
  extends Directives {
      ...
      path("help") {
        hostName { hn =>
          ...
          val routeApis = Map[String, (String, String)](
            "delayedResultUpload" -> ("POST", httpPrefix +  hn + localhostPortConfig  + "/delayed/result")
          )
          complete(routeApis)
        }
      } ~
            ...
            entity(as[JObject]) { jObject =>
              optionalCookie("commToken") {
                ...
                case Some(commToken) =>
                  ...
                  val response = (delayedDiscountActor ? JObjectTriadResult(jObject))
                    .mapTo[TransOk]
